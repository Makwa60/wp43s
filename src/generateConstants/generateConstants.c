...
void generateAllConstants(void) {
  ...
  generateConstantArray("a0",            "+5.291772105440000000000000000000000000000000000000000000e-11"); c++; //  cnst1
  ...
  generateConstantArray("me",            "+9.109383713900000000000000000000000000000000000000000000e-31"); c++; // cnst23

  generateConstantArray("MMoon",         "+7.349000000000000000000000000000000000000000000000000000e+22"); c++; // cnst24
  generateConstantArray("mn",            "+1.674927500560000000000000000000000000000000000000000000e-27"); c++; // cnst25
  generateConstantArray("mnOnmp",        "+1.001378419460000000000000000000000000000000000000000000e+00"); c++; // cnst26
  generateConstantArray("mp",            "+1.672621925950000000000000000000000000000000000000000000e-27"); c++; // cnst27
  generateConstantArray("mPL",           "+2.176434000000000000000000000000000000000000000000000000e-08"); c++; // cnst28
  generateConstantArray("mpOnme",        "+1.836152673426000000000000000000000000000000000000000000e+03"); c++; // cnst29

  generateConstantArray("mu",            "+1.660539068920000000000000000000000000000000000000000000e-27"); c++; // cnst30
  generateConstantArray("muc2",          "+1.492418087680000000000000000000000000000000000000000000e-10"); c++; // cnst31
  ...
  generateConstantArray("re",            "+2.817940320500000000000000000000000000000000000000000000e-15"); c++; // cnst39
  ...
  generateConstantArray("RInfinity",     "+1.0973731568157000000000000000000000000000000000000000000e+07"); c++; // cnst42
  ...
  generateConstantArray("TP",            "+1.416784000000000000000000000000000000000000000000000000e+32"); c++; // cnst51
  generateConstantArray("tPL",           "+5.391247000000000000000000000000000000000000000000000000e-44"); c++; // cnst52
  generateConstantArray("Vm",            "+2.241396954501413773501110288675055514433752775721687639e-02"); c++; // cnst53

  generateConstantArray("Z0",            "+3.767303134120000000000000000000000000000000000000000000e+02"); c++; // cnst54 mu0 * c
  generateConstantArray("alpha",         "+7.297352564300000000000000000000000000000000000000000000e-03"); c++; // cnst55
  generateConstantArray("gamma",         "+6.674300000000000000000000000000000000000000000000000000e-11"); c++; // cnst56 gamma = gEarth --> is this duplicate constant needed?
  ...
  generateConstantArray("gammap",        "+2.675221870800000000000000000000000000000000000000000000e+08"); c++; // cnst58
  generateConstantArray("DELTAvcs",      "+9.192631770000000000000000000000000000000000000000000000e+09"); c++; // cnst59 per definition

  generateConstantArray("epsilon0",      "+8.854187818800000000000000000000000000000000000000000000e-12"); c++; // cnst60
  generateConstantArray("lambdaC",       "+2.426310235380000000000000000000000000000000000000000000e-12"); c++; // cnst61
  generateConstantArray("lambdaCn",      "+1.319590903820000000000000000000000000000000000000000000e-15"); c++; // cnst62
  generateConstantArray("lambdaCp",      "+1.321409853600000000000000000000000000000000000000000000e-15"); c++; // cnst63
  generateConstantArray("mu0",           "+1.256637061270000000000000000000000000000000000000000000e-06"); c++; // cnst64
  generateConstantArray("muB",           "+9.274010065700000000000000000000000000000000000000000000e-24"); c++; // cnst65

  generateConstantArray("mue",           "-9.284764691700000000000000000000000000000000000000000000e-24"); c++; // cnst66
  generateConstantArray("mueOnmuB",      "-1.001159652180460000000000000000000000000000000000000000e+00"); c++; // cnst67
  generateConstantArray("mun",           "-9.662365300000000000000000000000000000000000000000000000e-27"); c++; // cnst68
  generateConstantArray("mup",           "+1.410606795450000000000000000000000000000000000000000000e-26"); c++; // cnst69
  generateConstantArray("muu",           "+5.050783739300000000000000000000000000000000000000000000e-27"); c++; // cnst70
  ...
  generateConstantArray("PointToMm",     "+3.527777777777777777777777777777777777777777777777777778e-01"); // cnst79       mm     = pt × 0.0254 / 72 × 1000
  generateConstantArray("InchToMm",      "+2.540000000000000000000000000000000000000000000000000000e+01"); // cnst80       mm     = inch × 0.0254 × 1000
  generateConstantArray("FtToM",         "+3.048000000000000000000000000000000000000000000000000000e-01"); // cnst81       m      = ft × 12 × 0.0254
  generateConstantArray("SfeetToM",      "+3.048006096012192024384048768097536195072390144780289561e-01"); // cnst82       m      = sfeetus × (1200 / 3937)
  generateConstantArray("YardToM",       "+9.144000000000000000000000000000000000000000000000000000e-01"); // cnst83       m      = yard × 3 × 12 × 0.0254
  generateConstantArray("FathomToM",     "+1.828800000000000000000000000000000000000000000000000000e+00"); // cnst84       m      = fathom × 6 × 12 × 0.0254
  generateConstantArray("MiToKm",        "+1.609344000000000000000000000000000000000000000000000000e+00"); // cnst85       km     = mile × 63360 × 0.0254 / 1000
  generateConstantArray("NmiToKm",       "+1.852000000000000000000000000000000000000000000000000000e+00"); // cnst86       km     = nmi × 1852 / 1000
  generateConstantArray("AuToM",         "+1.495978707000000000000000000000000000000000000000000000e+11"); // cnst87       m      = au × 149597870700
  generateConstantArray("LyToM",         "+9.460730472580800000000000000000000000000000000000000000e+15"); // cnst88       m      = ly × 299792458 × 3600 × 24 × 365.25
  generateConstantArray("PcToM",         "+3.085677581491367278913937957796471610731921160409179801e+16"); // cnst89       m      = pc × 149597870700 × 648000 / pi
  generateConstantArray("LiToM",         "+5.000000000000000000000000000000000000000000000000000000e+02"); // cnst90       m      = lǐ × 500
  generateConstantArray("YinToM",        "+3.000000000000000000000000000000000000000000000000000000e-02"); // cnst91       m      = yǐn / 0.03
  generateConstantArray("CunToM",        "+3.000000000000000000000000000000000000000000000000000000e+01"); // cnst92       m      = cùn / 30
  generateConstantArray("ZhangToM",      "+3.000000000000000000000000000000000000000000000000000000e-01"); // cnst93       m      = zhàng / 0.3
  generateConstantArray("FenToM",        "+3.000000000000000000000000000000000000000000000000000000e+02"); // cnst94       m      = fēn / 300
  generateConstantArray("MiToM",         "+1.609344000000000000000000000000000000000000000000000000e+03"); // cnst95       m      = mile × 63360 × 0.0254
  generateConstantArray("NmiToM",        "+1.852000000000000000000000000000000000000000000000000000e+03"); // cnst96       m      = nmi × 1852
  generateConstantArray("InchToM",       "+2.540000000000000000000000000000000000000000000000000000e-02"); // cnst97       m      = inch × 0.0254
  generateConstantArray("PointToM",      "+3.527777777777777777777777777777777777777777777777777778e-04"); // cnst98       m      = pt × 0.0254 / 72

  generateConstantArray("AcreToHa",      "+4.046856422400000000000000000000000000000000000000000000e-01"); // cnst99       ha     = acre × 0.0254² × 12² × 43560 / 10000
  generateConstantArray("AcreusToHa",    "+4.046872609874252006568529266090790246096621225500515517e-01"); // cnst100      ha     = acreus × (1200 / 3937)² × 43560 / 10000
  generateConstantArray("MuToM2",        "+1.500000000000000000000000000000000000000000000000000000e-03"); // cnst101      m²     = mǔ / 0.0015
  generateConstantArray("AcreToM2",      "+4.046856422400000000000000000000000000000000000000000000e+04"); // cnst102      m²     = acre × 0.0254² × 12² × 43560
  generateConstantArray("AcreusToM2",    "+4.046872609874252006568529266090790246096621225500515517e+04"); // cnst103      m²     = acreus × (1200 / 3937)² × 43560

  generateConstantArray("FlozukToMl",    "+2.841306250000000000000000000000000000000000000000000000e+01"); // cnst104      ml     = flozuk × 4.54609e-3 / 160 × 1000000
  generateConstantArray("FlozusToMl",    "+2.957352956250000000000000000000000000000000000000000000e+01"); // cnst105      ml     = flozus × 231 × 0.0254³ / 128 × 1000000
  generateConstantArray("GalusToL",      "+3.785411784000000000000000000000000000000000000000000000e+00"); // cnst106      l      = galus × 231 × 0.0254³ × 1000
  generateConstantArray("GalukToL",      "+4.546090000000000000000000000000000000000000000000000000e+00"); // cnst107      l      = galuk × 4.54609e-3 × 1000
  generateConstantArray("QuartToL",      "+1.136522500000000000000000000000000000000000000000000000e+00"); // cnst108      l      = quart × 4.54609e-3 / 4 × 1000
  ...
  generateConstantArray("FlozukToM3",    "+2.841306250000000000000000000000000000000000000000000000e-05"); // cnst110      m³     = flozuk × 4.54609e-3 / 160
  ...
  generateConstantArray("LbToKg",        "+4.535923700000000000000000000000000000000000000000000000e-01"); // cnst118      kg     = lb × 0.45359237     (1 lb (pound) = 16 oz)
  generateConstantArray("StoneToKg",     "+6.350293180000000000000000000000000000000000000000000000e+00"); // cnst119      kg     = stone × 14 × 0.45359237
  generateConstantArray("ShortcwtToKg",  "+4.535923700000000000000000000000000000000000000000000000e+01"); // cnst120      kg     = short cwt × 100 × 0.45359237 (short cwt = short hundredweight)
  ...
  generateConstantArray("CaratToKg",     "+2.000000000000000000000000000000000000000000000000000000e-04"); // cnst124      kg     = carat × 0.0002
  generateConstantArray("OzToKg",        "+2.834952312500000000000000000000000000000000000000000000e-02"); // cnst125      kg     = oz × (0.45359237 / 16)
  generateConstantArray("TrozToKg",      "+3.110347680000000000000000000000000000000000000000000000e-02"); // cnst126      kg     = tr.oz × 0.45359237 × 175 / 12

  generateConstantArray("CalToJ",        "+4.186800000000000000000000000000000000000000000000000000e+00"); // cnst127      joule  = calorie × 4.1868
  generateConstantArray("BtuToJ",        "+1.055055852620000000000000000000000000000000000000000000e+03"); // cnst128      joule  = Btu × 1055.05585262
  generateConstantArray("WhToJ",         "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst129      joule  = Wh × 3600

  generateConstantArray("LbfToN",        "+4.448221615260500000000000000000000000000000000000000000e+00"); // cnst130      newton = lbf × 9.80665 × 0.45359237

  generateConstantArray("TorrToPa",      "+1.333223684210526315789473684210526315789473684210526316e+02"); // cnst131      pascal = torr × 101325 / 760
  #if (MMHG_PA_133_3224 == 1)
    generateConstantArray("MmhgToPa",    "+1.333224000000000000000000000000000000000000000000000000e+02"); // cnst132      pascal = mm.Hg × 133.3224
    generateConstantArray("InhgToPa",    "+3.386388960000000000000000000000000000000000000000000000e+03"); // cnst133      pascal = in.Hg × 133.3224 × 25.4
  #else // (MMHG_PA_133_3224 == 0)
    generateConstantArray("MmhgToPa",    "+1.333223874150000000000000000000000000000000000000000000e+02"); // cnst132      pascal = mm.Hg × 13.5951 × 9.80665
    generateConstantArray("InhgToPa",    "+3.386388640341000000000000000000000000000000000000000000e+03"); // cnst133      pascal = in.Hg × 13.5951 × 9.80665 × 2.54
  #endif // (MMHG_PA_133_3224 == 1)
  generateConstantArray("PsiToPa",       "+6.894757293168361336722673445346890693781387562775125550e+03"); // cnst134      pascal = psi × 0.45359237 × 9.80665 / 0.0254²
  generateConstantArray("BarToPa",       "+1.000000000000000000000000000000000000000000000000000000e+05"); // cnst135      pascal = bar  × 100000
  generateConstantArray("AtmToPa",       "+1.013250000000000000000000000000000000000000000000000000e+05"); // cnst136      pascal = atm × 101325

  generateConstantArray("HpmToW",        "+7.354987500000000000000000000000000000000000000000000000e+02"); // cnst137      watt   = HPM × 75 × 9.80665
  generateConstantArray("HpukToW",       "+7.456998715822702200000000000000000000000000000000000000e+02"); // cnst138      watt   = HPUK × 550 × 0.3048 × 9.80665 × 0.45359237
  generateConstantArray("HpeToW",        "+7.460000000000000000000000000000000000000000000000000000e+02"); // cnst139      watt   = HPE × 746

  generateConstantArray("YearToS",       "+3.155695200000000000000000000000000000000000000000000000e+07"); // cnst140      second = year  × (365.2425 × 24 × 3600)

  generateConstantArray("LbfftToNm",     "+1.355817948331400400000000000000000000000000000000000000e+00"); // cnst141      Nm = lbf×ft × 9.80665 × 0.45359237 × 12 × 0.0254

  generateConstantArray("_108",          "-1.080000000000000000000000000000000000000000000000000000e+02"); // cnst142
  generateConstantArray("_4",            "-4.000000000000000000000000000000000000000000000000000000e+00"); // cnst143
  generateConstantArray("_1",            "-1.000000000000000000000000000000000000000000000000000000e+00"); // cnst144
  generateConstantArray("_1oneE",        "-3.678794411714423215955237701614608674458111310317678345e-01"); // cnst145
  generateConstantArray("_7on20",        "-3.500000000000000000000000000000000000000000000000000000e-01"); // cnst146
  generateConstantArray("_1on4",         "-2.500000000000000000000000000000000000000000000000000000e-01"); // cnst147
  generateConstantArray("1e_49",         "+1.000000000000000000000000000000000000000000000000000000e-49"); // cnst148
  generateConstantArray("1e_37",         "+1.000000000000000000000000000000000000000000000000000000e-37"); // cnst149
  generateConstantArray("1e_24",         "+1.000000000000000000000000000000000000000000000000000000e-24"); // cnst150
  generateConstantArray("1e_16",         "+1.000000000000000000000000000000000000000000000000000000e-16"); // cnst151
  generateConstantArray("1e_6",          "+1.000000000000000000000000000000000000000000000000000000e-06"); // cnst152
  generateConstantArray("1e_4",          "+1.000000000000000000000000000000000000000000000000000000e-04"); // cnst153
  generateConstantArray("9on160",        "+5.625000000000000000000000000000000000000000000000000000e-02"); // cnst154
  generateConstantArray("1on10",         "+1.000000000000000000000000000000000000000000000000000000e-01"); // cnst155
  generateConstantArray("1on8",          "+1.250000000000000000000000000000000000000000000000000000e-01"); // cnst156
  generateConstantArray("5on24",         "+2.083333333333333333333333333333333333333333333333333333e-01"); // cnst157
  generateConstantArray("111on500",      "+2.220000000000000000000000000000000000000000000000000000e-01"); // cnst158
  generateConstantArray("1on4",          "+2.500000000000000000000000000000000000000000000000000000e-01"); // cnst159
  generateConstantArray("33on125",       "+2.640000000000000000000000000000000000000000000000000000e-01"); // cnst160
  generateConstantArray("1on3",          "+3.333333333333333333333333333333333333333333333333333333e-01"); // cnst161
  generateConstantArray("1on2",          "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst162
  generateConstantArray("2on3",          "+6.666666666666666666666666666666666666666666666666666667e-01"); // cnst163
  ...
  generateConstantArray("19on10",        "+1.900000000000000000000000000000000000000000000000000000e+00"); // cnst175
  generateConstantArray("2",             "+2.000000000000000000000000000000000000000000000000000000e+00"); // cnst176
  generateConstantArray("ln10",          "+2.302585092994045684017991454684364207601101488628772976e+00"); // cnst177
  generateConstantArray("3piOn4",        "+2.356194490192344928846982537459627163147877049531329366e+00"); // cnst178
  generateConstantArray("3",             "+3.000000000000000000000000000000000000000000000000000000e+00"); // cnst179
  generateConstantArray("pi",            "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst180

  generateConstantArray("4",             "+4.000000000000000000000000000000000000000000000000000000e+00"); // cnst181
  generateConstantArray("3piOn2",        "+4.712388980384689857693965074919254326295754099062658731e+00"); // cnst182
  generateConstantArray("5",             "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst183
  generateConstantArray("6",             "+6.000000000000000000000000000000000000000000000000000000e+00"); // cnst184
  generateConstantArray("2pi",           "+6.283185307179586476925286766559005768394338798750211642e+00"); // cnst185
  generateConstantArray("7",             "+7.000000000000000000000000000000000000000000000000000000e+00"); // cnst186
  generateConstantArray("8",             "+8.000000000000000000000000000000000000000000000000000000e+00"); // cnst187
  generateConstantArray("9",             "+9.000000000000000000000000000000000000000000000000000000e+00"); // cnst188
  generateConstantArray("10",            "+1.000000000000000000000000000000000000000000000000000000e+01"); // cnst189
  generateConstantArray("12",            "+1.200000000000000000000000000000000000000000000000000000e+01"); // cnst190
  generateConstantArray("16",            "+1.600000000000000000000000000000000000000000000000000000e+01"); // cnst191
  generateConstantArray("20",            "+2.000000000000000000000000000000000000000000000000000000e+01"); // cnst192
  //generateConstantArray("21",            "+2.100000000000000000000000000000000000000000000000000000e+01"); // cnst193     used for Lanczos N=22
  //generateConstantArray("gammaR",        "+2.311891000000000000000000000000000000000000000000000000e+01"); // cnst194     used for Lanczos N=22
  generateConstantArray("24",            "+2.400000000000000000000000000000000000000000000000000000e+01"); // cnst193
  generateConstantArray("25",            "+2.500000000000000000000000000000000000000000000000000000e+01"); // cnst194
  generateConstantArray("27",            "+2.700000000000000000000000000000000000000000000000000000e+01"); // cnst195
  generateConstantArray("29",            "+2.900000000000000000000000000000000000000000000000000000e+01"); // cnst196     used for Lanczos N=30
  generateConstantArray("30",            "+3.000000000000000000000000000000000000000000000000000000e+01"); // cnst197
  generateConstantArray("gammaR",        "+3.143188335932791233062744140625000000000000000000000000e+01"); // cnst198     used for Lanczos N=30
  generateConstantArray("32",            "+3.200000000000000000000000000000000000000000000000000000e+01"); // cnst199
  generateConstantArray("36",            "+3.600000000000000000000000000000000000000000000000000000e+01"); // cnst200
  generateConstantArray("47",            "+4.700000000000000000000000000000000000000000000000000000e+01"); // cnst201
  generateConstantArray("54",            "+5.400000000000000000000000000000000000000000000000000000e+01"); // cnst202
  generateConstantArray("180onPi",       "+5.729577951308232087679815481410517033240547246656432155e+01"); // cnst203
  generateConstantArray("60",            "+6.000000000000000000000000000000000000000000000000000000e+01"); // cnst204
  generateConstantArray("200onPi",       "+6.366197723675813430755350534900574481378385829618257950e+01"); // cnst205
  generateConstantArray("125",           "+1.250000000000000000000000000000000000000000000000000000e+02"); // cnst206
  generateConstantArray("150",           "+1.500000000000000000000000000000000000000000000000000000e+02"); // cnst207
  generateConstantArray("205",           "+2.050000000000000000000000000000000000000000000000000000e+02"); // cnst208
  generateConstantArray("360",           "+3.600000000000000000000000000000000000000000000000000000e+02"); // cnst209
  generateConstantArray("400",           "+4.000000000000000000000000000000000000000000000000000000e+02"); // cnst210
  generateConstantArray("600",           "+6.000000000000000000000000000000000000000000000000000000e+02"); // cnst211
  generateConstantArray("1000",          "+1.000000000000000000000000000000000000000000000000000000e+03"); // cnst212
  generateConstantArray("3200onPi",      "+1.018591635788130148920856085584091917020541732738921272e+03"); // cnst213
  generateConstantArray("1260",          "+1.260000000000000000000000000000000000000000000000000000e+03"); // cnst214
  generateConstantArray("1680",          "+1.680000000000000000000000000000000000000000000000000000e+03"); // cnst215
  generateConstantArray("2100",          "+2.100000000000000000000000000000000000000000000000000000e+03"); // cnst216
  generateConstantArray("2520",          "+2.520000000000000000000000000000000000000000000000000000e+03"); // cnst217
  generateConstantArray("2916",          "+2.916000000000000000000000000000000000000000000000000000e+03"); // cnst218
  generateConstantArray("3600",          "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst219
  generateConstantArray("6000",          "+6.000000000000000000000000000000000000000000000000000000e+03"); // cnst220
  generateConstantArray("6400",          "+6.400000000000000000000000000000000000000000000000000000e+03"); // cnst221
  generateConstantArray("9000",          "+9.000000000000000000000000000000000000000000000000000000e+03"); // cnst222
  generateConstantArray("9999",          "+9.999000000000000000000000000000000000000000000000000000e+03"); // cnst223
  generateConstantArray("10000",         "+1.000000000000000000000000000000000000000000000000000000e+04"); // cnst224
  generateConstantArray("25200",         "+2.520000000000000000000000000000000000000000000000000000e+04"); // cnst225
  generateConstantArray("42000",         "+4.200000000000000000000000000000000000000000000000000000e+04"); // cnst226
  generateConstantArray("73766",         "+7.376600000000000000000000000000000000000000000000000000e+04"); // cnst227
  generateConstantArray("86400",         "+8.640000000000000000000000000000000000000000000000000000e+04"); // cnst228
  generateConstantArray("1e6",           "+1.000000000000000000000000000000000000000000000000000000e+06"); // cnst229
  generateConstantArray("1e8",           "+1.000000000000000000000000000000000000000000000000000000e+08"); // cnst230
  generateConstantArray("1e9",           "+1.000000000000000000000000000000000000000000000000000000e+09"); // cnst231
  generateConstantArray("2p32",          "+4.294967296000000000000000000000000000000000000000000000e+09"); // cnst232
  generateConstantArray("1e32",          "+1.000000000000000000000000000000000000000000000000000000e+32"); // cnst233
  generateConstantArray("1e_6143",       "+1.000000000000000000000000000000000000000000000000000000e-6143"); // cnst234
  generateConstantArray("1e_10000",      "+1.000000000000000000000000000000000000000000000000000000e-10000"); // cnst235
  generateConstantArray("995on1000",     "+9.950000000000000000000000000000000000000000000000000000e-01"); // cnst236
  generateConstantArray("1e_32",         "+1.000000000000000000000000000000000000000000000000000000e-32"); // cnst237
  generateConstantArray("TspukToMl",     "+5.919388020833333333333333333333333333333333333333333333e+00"); // cnst238      ml     = tspuk    × 5.9193880208333...
  generateConstantArray("TbspukToMl",    "+1.775816406250000000000000000000000000000000000000000000e+01"); // cnst239      ml     = tbspuk   × 17.7581640625
  generateConstantArray("CupukToMl",     "+2.841306250000000000000000000000000000000000000000000000e+02"); // cnst240      ml     = cupuk    × 284.130625
  generateConstantArray("PintukToMl",    "+5.682612500000000000000000000000000000000000000000000000e+02"); // cnst241      ml     = pintuk   × 568.26125
  generateConstantArray("QuartukToMl",   "+1.136522500000000000000000000000000000000000000000000000e+03"); // cnst242      ml     = quart    × 4.54609e-3 / 4 × 1000000
  generateConstantArray("GalukToMl",     "+4.546090000000000000000000000000000000000000000000000000e+03"); // cnst243      ml     = galuk    × 4.54609e-3 × 1000000
  generateConstantArray("TspusToMl",     "+4.928921593750000000000000000000000000000000000000000000e+00"); // cnst244      ml     = tspus    × 4.92892159375
  generateConstantArray("TbspusToMl",    "+1.478676478125000000000000000000000000000000000000000000e+01"); // cnst245      ml     = tbspus   × 14.78676478125
  generateConstantArray("CupusToMl",     "+2.365882365000000000000000000000000000000000000000000000e+02"); // cnst246      ml     = cupus    × 236.5882365
  generateConstantArray("PintusToMl",    "+4.731764730000000000000000000000000000000000000000000000e+02"); // cnst247      ml     = pintus   × 473.176473
  generateConstantArray("QuartusToMl",   "+9.463529460000000000000000000000000000000000000000000000e+02"); // cnst248      ml     = quartus  × 57.75 × 0.0254³ x 1000000
  generateConstantArray("QuartusToL",    "+9.463529460000000000000000000000000000000000000000000000e-01"); // cnst249      l      = quartus  × 57.75 × 0.0254³ x 1000
  generateConstantArray("GalusToMl",     "+3.785411784000000000000000000000000000000000000000000000e+03"); // cnst250      ml     = galus    × 231   × 0.0254³ x 1000000
  generateConstantArray("TspmToMl",      "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst251      ml     = tspm     × 5.00000
  generateConstantArray("TbspmToMl",     "+1.500000000000000000000000000000000000000000000000000000e+01"); // cnst252      ml     = tbspm    × 15.0000
  generateConstantArray("CupmToMl",      "+2.500000000000000000000000000000000000000000000000000000e+02"); // cnst253      ml     = cupm     × 250.000
  generateConstantArray("TbspauToMl",    "+2.000000000000000000000000000000000000000000000000000000e+01"); // cnst254      ml     = tbspau   × 20.0000
  generateConstantArray("PintdryToL",    "+5.506104713575000000000000000000000000000000000000000000e-01"); // cnst255      l      = pintdry  × 0.5506104713575
  generateConstantArray("QuartdryToL",   "+1.101220942715000000000000000000000000000000000000000000e+00"); // cnst256      l      = quartdry × 1.101220942715
  generateConstantArray("GaldryToL",     "+4.404883770860000000000000000000000000000000000000000000e+00"); // cnst257      l      = galdry   × 4.40488377086
  generateConstantArray("PeckusToL",     "+8.809767541720000000000000000000000000000000000000000000e+00"); // cnst258      l      = peckus   × 8.80976754172
  generateConstantArray("BushelusToL",   "+3.523907016688000000000000000000000000000000000000000000e+01"); // cnst259      l      = bushelus × 35.23907016688  (NIST)
  generateConstantArray("Inch3ToMl",     "+1.638706400000000000000000000000000000000000000000000000e+01"); // cnst260      ml     = inch^3   × 16.387064       (NIST)
  generateConstantArray("Ft3ToL",        "+2.831684659200000000000000000000000000000000000000000000e+01"); // cnst261      l      = ft^3     × 28.316846592    (NIST)
  generateConstantArray("FlozukToInch3", "+1.733871454947634304717428332494460000000000000000000000e+00"); // cnst262      inch^3 = flozuk   × 28.4130625 / 2.54³
  generateConstantArray("FlozusToInch3", "+1.804687500000000000000000000000000000000000000000000000e+00"); // cnst263      inch^3 = flozus   × 1.8046875
  generateConstantArray("Ft3ToGaluk",    "+6.228835459042825812951349401353691000000000000000000000e+00"); // cnst264      galuk  = ((ft × 12 × 2.54)^3 / 1000) / (4.54609e-3 × 1000, definition UK) x
  generateConstantArray("Ft3ToGalUS",    "+7.480519480519480519480519480519480000000000000000000000e+00"); // cnst265      galus  = feet3 * (1/(231 × 0.0254³ × 1000)) * (12*0.254)³
  generateConstantArray("QuartusToM3",   "+9.463529460000000000000000000000000000000000000000000000e-04"); // cnst266      m³     = quartus  × 231 × 0.0254³ / 40.0254³
//  if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
  ...
  // source: https://www.vttoth.com/FILES/lanczos.tgz
  generateConstantArray51("gammaC00",      "+2.506628274631000502415765284811045253006986740036318756703152698088652909808141644264478158114643"); // cnst267
  generateConstantArray51("gammaC01",      "+90795313438554.12585760728412902207185494121818510422990562390199419594778421509725059250660477478"); // cnst268
  generateConstantArray51("gammaC02",      "-966788810976616.3165238613163922965882171247384377771690792930892852741666385482155753752741846215"); // cnst269
  generateConstantArray51("gammaC03",      "+4800584619652903.130520636355077553806266769741879659498830407152738371167934130745600044623388068"); // cnst270
  generateConstantArray51("gammaC04",      "-14765868961297540.27064904092821149556100711736690133148186310232345861145647352272488652687903457"); // cnst271
  generateConstantArray51("gammaC05",      "+31520009430682538.17457937985920649010992386050798974102315423368438398460245896094052856122203775"); // cnst272
  generateConstantArray51("gammaC06",      "-49582365010416661.09326008795178786489253227385559918969410907133030684376335533671631031226780693"); // cnst273
  generateConstantArray51("gammaC07",      "+59568927402677002.04904143939675965126139196724493177865501398609411221897535112902272116361691451"); // cnst274
  generateConstantArray51("gammaC08",      "-55906342816448463.66899438215049258933959227649607458054232171494476786360919120606779433224203661"); // cnst275
  generateConstantArray51("gammaC09",      "+41579705761882718.81836872026846865595091094440150772658809279820390684724850793073271091274939857"); // cnst276
  generateConstantArray51("gammaC10",      "-24720590950576683.15802797249415999638807846332716128189191944706094647468094435934791589404852933"); // cnst277
  generateConstantArray51("gammaC11",      "+11801286985635343.58575847383935274746074763562173560062003882605616928057523864988435228756829000"); // cnst278
  generateConstantArray51("gammaC12",      "-4528196760270806.995369621666200554735964133084677928395568191894066774034949226537269118163329489"); // cnst279
  generateConstantArray51("gammaC13",      "+1393637064384668.917157722908202033726675908093527246704252104958408303381715387740313008845141904"); // cnst280
  generateConstantArray51("gammaC14",      "-342325016766888.5479936955467723387327600568400337988293619211103486673308239810777313598627315000"); // cnst281
  generateConstantArray51("gammaC15",      "+66578560073752.67246403368251720854813222816576859988564202476567602534436134110254400104404942690"); // cnst282
  generateConstantArray51("gammaC16",      "-10138544793022.41604607361389789580035078467043905508354626409172360042771928364449573017241522124"); // cnst283
  generateConstantArray51("gammaC17",      "+1190960562789.022438954352946350698116714288406439572882388541923119338089933765849690628214181895"); // cnst284
  generateConstantArray51("gammaC18",      "-105857166683.4123935373722836022279857082446927458625016646619942749346232137590435016079220044575"); // cnst285
  generateConstantArray51("gammaC19",      "+6945291327.796451124796820522079456356395826950647769057580507091796596412483352351183618549540841"); // cnst286
  generateConstantArray51("gammaC20",      "-325841232.8464260929688459624630603325071713695605050282752915466327762939975965122976740632863654"); // cnst287
  generateConstantArray51("gammaC21",      "+10491089.04183867431328778246866673594812558539972730107563825597306012268992151854201423652554125"); // cnst288
  generateConstantArray51("gammaC22",      "-219652.9521075940388150629624539015343302845601375714110391179944415326942002537588845815284375311"); // cnst289
  generateConstantArray51("gammaC23",      "+2782.640500292405732331071582785081107276742114500310058012342169019346915906467717358069437084413"); // cnst290
  generateConstantArray51("gammaC24",      "-19.31769482393126228999283928711817461826517031491190754697778125203682359806685621504868658022287"); // cnst291
  generateConstantArray51("gammaC25",      "+0.063813226038364736490677427053094184825560153837400637303741419820260651820063721448175876091640"); // cnst292
  generateConstantArray51("gammaC26",      "-0.000081213345051388882076245151519040492731624505633194015094047296992491188162952243561414155660"); // cnst293
  generateConstantArray51("gammaC27",      "+0.000000028425884712306176285600873249949436913248124688912080334562388569408577988201187292514310"); // cnst294
  generateConstantArray51("gammaC28",      "-0.000000000001515686855682081535390179935440192138970398946754350022046202956603870401666503058594"); // cnst295
  generateConstantArray51("gammaC29",      "+0.000000000000000003727184110478268994444830355775133199862144652804253659035860755294511160286895"); // cnst296

  // Array angle180
  generateConstantArray51("pi_51",         "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst297
  generateConstantArray51("200",           "+2.000000000000000000000000000000000000000000000000000000e+02"); // cnst298
  generateConstantArray51("180",           "+1.800000000000000000000000000000000000000000000000000000e+02"); // cnst299
  generateConstantArray51("3200",          "+3.200000000000000000000000000000000000000000000000000000e+03"); // cnst300

  // Array angle90
  generateConstantArray51("piOn2_51",      "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst301
  generateConstantArray51("100",           "+1.000000000000000000000000000000000000000000000000000000e+02"); // cnst302
  generateConstantArray51("90",            "+9.000000000000000000000000000000000000000000000000000000e+01"); // cnst303
  generateConstantArray51("1600",          "+1.600000000000000000000000000000000000000000000000000000e+03"); // cnst304

  // Array angle45
  generateConstantArray51("piOn4_51",      "+7.853981633974483096156608458198757210492923498437764552e-01"); // cnst305
  generateConstantArray51("50",            "+5.000000000000000000000000000000000000000000000000000000e+01"); // cnst306
  generateConstantArray51("45",            "+4.500000000000000000000000000000000000000000000000000000e+01"); // cnst307
  generateConstantArray51("800",           "+8.000000000000000000000000000000000000000000000000000000e+02"); // cnst308
  
  // MultPi constants
  generateConstantArray51("1_51",          "+1.000000000000000000000000000000000000000000000000000000e+00"); // cnst309
  generateConstantArray51("1on2_51",       "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst310
  generateConstantArray51("1on4_51",       "+2.500000000000000000000000000000000000000000000000000000e-01"); // cnst311
  ...
  generateConstantArray34("_4712",       "-4.712000000000000000000000000000000000000000000000000000e+03"); // cnst313
  generateConstantArray34("_1",          "-1.000000000000000000000000000000000000000000000000000000e+00"); // cnst314
  generateConstantArray34("0",           "+0.000000000000000000000000000000000000000000000000000000e+00"); // cnst315
  generateConstantArray34("1e_32",       "+1.000000000000000000000000000000000000000000000000000000e-32"); // cnst316
  generateConstantArray34("1e_6",        "+1.000000000000000000000000000000000000000000000000000000e-06"); // cnst317
  generateConstantArray34("1e_4",        "+1.000000000000000000000000000000000000000000000000000000e-04"); // cnst318
  generateConstantArray34("1on100",      "+1.000000000000000000000000000000000000000000000000000000e-02"); // cnst319
  generateConstantArray34("1on10",       "+1.000000000000000000000000000000000000000000000000000000e-01"); // cnst320
  generateConstantArray34("1on3",        "+3.333333333333333333333333333333333333333333333333333333e-01"); // cnst321
  generateConstantArray34("1on2",        "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst322
  generateConstantArray34("2on3",        "+6.666666666666666666666666666666666666666666666666666667e-01"); // cnst323
  generateConstantArray34("ln2",         "+6.931471805599453094172321214581765680755001343602552541e-01"); // cnst324
  generateConstantArray34("1",           "+1.000000000000000000000000000000000000000000000000000000e+00"); // cnst325
  generateConstantArray34("piOn2",       "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst326
  generateConstantArray34("2",           "+2.000000000000000000000000000000000000000000000000000000e+00"); // cnst327
  generateConstantArray34("ln10",        "+2.302585092994045684017991454684364207601101488628772976e+00"); // cnst328
  generateConstantArray34("3",           "+3.000000000000000000000000000000000000000000000000000000e+00"); // cnst329
  generateConstantArray34("pi",          "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst330
  generateConstantArray34("4",           "+4.000000000000000000000000000000000000000000000000000000e+00"); // cnst331
  generateConstantArray34("5",           "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst332
  generateConstantArray34("2pi",         "+6.283185307179586476925286766559005768394338798750211642e+00"); // cnst333
  generateConstantArray34("7",           "+7.000000000000000000000000000000000000000000000000000000e+00"); // cnst334
  generateConstantArray34("9",           "+9.000000000000000000000000000000000000000000000000000000e+00"); // cnst335
  generateConstantArray34("10",          "+1.000000000000000000000000000000000000000000000000000000e+01"); // cnst336
  generateConstantArray34("12",          "+1.200000000000000000000000000000000000000000000000000000e+01"); // cnst337
  generateConstantArray34("14",          "+1.400000000000000000000000000000000000000000000000000000e+01"); // cnst338
  generateConstantArray34("24",          "+2.400000000000000000000000000000000000000000000000000000e+01"); // cnst339
  generateConstantArray34("28",          "+2.800000000000000000000000000000000000000000000000000000e+01"); // cnst340
  generateConstantArray34("31",          "+3.100000000000000000000000000000000000000000000000000000e+01"); // cnst341
  generateConstantArray34("38",          "+3.800000000000000000000000000000000000000000000000000000e+01"); // cnst342
  generateConstantArray34("47",          "+4.700000000000000000000000000000000000000000000000000000e+01"); // cnst343
  generateConstantArray34("60",          "+6.000000000000000000000000000000000000000000000000000000e+01"); // cnst344
  generateConstantArray34("100",         "+1.000000000000000000000000000000000000000000000000000000e+02"); // cnst345
  generateConstantArray34("153",         "+1.530000000000000000000000000000000000000000000000000000e+02"); // cnst346
  generateConstantArray34("275",         "+2.750000000000000000000000000000000000000000000000000000e+02"); // cnst347
  generateConstantArray34("367",         "+3.670000000000000000000000000000000000000000000000000000e+02"); // cnst348
  generateConstantArray34("400",         "+4.000000000000000000000000000000000000000000000000000000e+02"); // cnst349
  generateConstantArray34("1000",        "+1.000000000000000000000000000000000000000000000000000000e+03"); // cnst350
  generateConstantArray34("1401",        "+1.401000000000000000000000000000000000000000000000000000e+03"); // cnst351
  generateConstantArray34("1461",        "+1.461000000000000000000000000000000000000000000000000000e+03"); // cnst352
  generateConstantArray34("3600",        "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst353
  generateConstantArray34("4716",        "+4.716000000000000000000000000000000000000000000000000000e+03"); // cnst354
  generateConstantArray34("4800",        "+4.800000000000000000000000000000000000000000000000000000e+03"); // cnst355
  generateConstantArray34("4900",        "+4.900000000000000000000000000000000000000000000000000000e+03"); // cnst356
  generateConstantArray34("5001",        "+5.001000000000000000000000000000000000000000000000000000e+03"); // cnst357
  generateConstantArray34("10000",       "+1.000000000000000000000000000000000000000000000000000000e+04"); // cnst358
  generateConstantArray34("20000",       "+2.000000000000000000000000000000000000000000000000000000e+04"); // cnst359
  generateConstantArray34("32075",       "+3.207500000000000000000000000000000000000000000000000000e+04"); // cnst360
  generateConstantArray34("43200",       "+4.320000000000000000000000000000000000000000000000000000e+04"); // cnst361
  generateConstantArray34("65535",       "+6.553500000000000000000000000000000000000000000000000000e+04"); // cnst362
  generateConstantArray34("86400",       "+8.640000000000000000000000000000000000000000000000000000e+04"); // cnst363
  generateConstantArray34("146097",      "+1.460970000000000000000000000000000000000000000000000000e+05"); // cnst364
  generateConstantArray34("274277",      "+2.742770000000000000000000000000000000000000000000000000e+05"); // cnst365
  generateConstantArray34("1e6",         "+1.000000000000000000000000000000000000000000000000000000e+06"); // cnst366
  generateConstantArray34("1729777",     "+1.729777000000000000000000000000000000000000000000000000e+06"); // cnst367
  generateConstantArray34("2p32",        "+4.294967296000000000000000000000000000000000000000000000e+09"); // cnst368
  generateConstantArray34("maxDate",     "+3.155695348699627200000000000000000000000000000000000000e+18"); // cnst369
  generateConstantArray34("maxTime",     "+3.600000000000000000000000000000000000000000000000000000e+19"); // cnst370  if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
