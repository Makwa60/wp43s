...
void generateAllConstants(void) {
  ...
  generateConstantArray("AcreToHa",      "+4.046856422400000000000000000000000000000000000000000000e-01"); // cnst98       ha     = acre × 0.0254² × 12² × 43560 / 10000
  generateConstantArray("AcreusToHa",    "+4.046872609874252006568529266090790246096621225500515517e-01"); // cnst99       ha     = acreus × (1200 / 3937)² × 43560 / 10000
  generateConstantArray("MuToM2",        "+1.500000000000000000000000000000000000000000000000000000e-03"); // cnst100      m²     = mǔ / 0.0015
  generateConstantArray("AcreToM2",      "+4.046856422400000000000000000000000000000000000000000000e+04"); // cnst101      m²     = acre × 0.0254² × 12² × 43560
  generateConstantArray("AcreusToM2",    "+4.046872609874252006568529266090790246096621225500515517e+04"); // cnst102      m²     = acreus × (1200 / 3937)² × 43560

  generateConstantArray("FlozukToMl",    "+2.841306250000000000000000000000000000000000000000000000e+01"); // cnst103      ml     = flozuk × 4.54609e-3 / 160 × 1000000
  generateConstantArray("FlozusToMl",    "+2.957352956250000000000000000000000000000000000000000000e+01"); // cnst104      ml     = flozus × 231 × 0.0254³ / 128 × 1000000
  generateConstantArray("GalusToL",      "+3.785411784000000000000000000000000000000000000000000000e+00"); // cnst105      l      = galus × 231 × 0.0254³ × 1000
  generateConstantArray("GalukToL",      "+4.546090000000000000000000000000000000000000000000000000e+00"); // cnst106      l      = galuk × 4.54609e-3 × 1000
  generateConstantArray("QuartToL",      "+1.136522500000000000000000000000000000000000000000000000e+00"); // cnst107      l      = quart × 4.54609e-3 / 4 × 1000
  generateConstantArray("BarrelToM3",    "+1.589872949280000000000000000000000000000000000000000000e-01"); // cnst108      m³     = barrel × 42 × 231 × 0.0254³
  generateConstantArray("FlozukToM3",    "+2.841306250000000000000000000000000000000000000000000000e-05"); // cnst109      m³     = flozuk × 4.54609e-3 / 160
  generateConstantArray("FlozusToM3",    "+2.957352956250000000000000000000000000000000000000000000e-05"); // cnst110      m³     = flozus × 231 × 0.0254³ / 128
  generateConstantArray("GalusToM3",     "+3.785411784000000000000000000000000000000000000000000000e-03"); // cnst111      m³     = galus × 231 × 0.0254³
  generateConstantArray("GalukToM3",     "+4.546090000000000000000000000000000000000000000000000000e-03"); // cnst112      m³     = galuk × 4.54609e-3
  generateConstantArray("QuartToM3",     "+1.136522500000000000000000000000000000000000000000000000e-03"); // cnst113      m³     = quart × 4.54609e-3 / 4

  generateConstantArray("CaratToG",      "+2.000000000000000000000000000000000000000000000000000000e-01"); // cnst114      g      = carat × 0.0002 × 1000
  generateConstantArray("OzToG",         "+2.834952312500000000000000000000000000000000000000000000e+01"); // cnst115      g      = oz × (0.45359237 / 16) × 1000
  generateConstantArray("TrozToG",       "+3.110347680000000000000000000000000000000000000000000000e+01"); // cnst116      g      = tr.oz × 0.45359237 × 175 / 12 × 1000
  generateConstantArray("LbToKg",        "+4.535923700000000000000000000000000000000000000000000000e-01"); // cnst117      kg     = lb × 0.45359237     (1 lb (pound) = 16 oz)
  generateConstantArray("StoneToKg",     "+6.350293180000000000000000000000000000000000000000000000e+00"); // cnst118      kg     = stone × 14 × 0.45359237
  generateConstantArray("ShortcwtToKg",  "+4.535923700000000000000000000000000000000000000000000000e+01"); // cnst119      kg     = short cwt × 100 × 0.45359237 (short cwt = short hundredweight)
  generateConstantArray("CwtToKg",       "+5.080234544000000000000000000000000000000000000000000000e+01"); // cnst120      kg     = cwt × 112 × 0.45359237       (cwt = long hundredweight)
  generateConstantArray("ShorttonToKg",  "+9.071847400000000000000000000000000000000000000000000000e+02"); // cnst121      kg     = short ton × 2000 × 0.45359237
  generateConstantArray("TonToKg",       "+1.016046908800000000000000000000000000000000000000000000e+03"); // cnst122      kg     = ton × 2240 × 0.45359237
  generateConstantArray("CaratToKg",     "+2.000000000000000000000000000000000000000000000000000000e-04"); // cnst123      kg     = carat × 0.0002
  generateConstantArray("OzToKg",        "+2.834952312500000000000000000000000000000000000000000000e-02"); // cnst124      kg     = oz × (0.45359237 / 16)
  generateConstantArray("TrozToKg",      "+3.110347680000000000000000000000000000000000000000000000e-02"); // cnst125      kg     = tr.oz × 0.45359237 × 175 / 12

  generateConstantArray("CalToJ",        "+4.186800000000000000000000000000000000000000000000000000e+00"); // cnst126      joule  = calorie × 4.1868
  generateConstantArray("BtuToJ",        "+1.055055852620000000000000000000000000000000000000000000e+03"); // cnst127      joule  = Btu × 1055.05585262
  generateConstantArray("WhToJ",         "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst128      joule  = Wh × 3600

  generateConstantArray("LbfToN",        "+4.448221615260500000000000000000000000000000000000000000e+00"); // cnst129      newton = lbf × 9.80665 × 0.45359237

  generateConstantArray("TorrToPa",      "+1.333223684210526315789473684210526315789473684210526316e+02"); // cnst130      pascal = torr × 101325 / 760
  #if (MMHG_PA_133_3224 == 1)
    generateConstantArray("MmhgToPa",    "+1.333224000000000000000000000000000000000000000000000000e+02"); // cnst131      pascal = mm.Hg × 133.3224
    generateConstantArray("InhgToPa",    "+3.386388960000000000000000000000000000000000000000000000e+03"); // cnst132      pascal = in.Hg × 133.3224 × 25.4
  #else // (MMHG_PA_133_3224 == 0)
    generateConstantArray("MmhgToPa",    "+1.333223874150000000000000000000000000000000000000000000e+02"); // cnst131      pascal = mm.Hg × 13.5951 × 9.80665
    generateConstantArray("InhgToPa",    "+3.386388640341000000000000000000000000000000000000000000e+03"); // cnst132      pascal = in.Hg × 13.5951 × 9.80665 × 2.54
  #endif // (MMHG_PA_133_3224 == 1)
  generateConstantArray("PsiToPa",       "+6.894757293168361336722673445346890693781387562775125550e+03"); // cnst133      pascal = psi × 0.45359237 × 9.80665 / 0.0254²
  generateConstantArray("BarToPa",       "+1.000000000000000000000000000000000000000000000000000000e+05"); // cnst134      pascal = bar  × 100000
  generateConstantArray("AtmToPa",       "+1.013250000000000000000000000000000000000000000000000000e+05"); // cnst135      pascal = atm × 101325

  generateConstantArray("HpmToW",        "+7.354987500000000000000000000000000000000000000000000000e+02"); // cnst136      watt   = HPM × 75 × 9.80665
  generateConstantArray("HpukToW",       "+7.456998715822702200000000000000000000000000000000000000e+02"); // cnst137      watt   = HPUK × 550 × 0.3048 × 9.80665 × 0.45359237
  generateConstantArray("HpeToW",        "+7.460000000000000000000000000000000000000000000000000000e+02"); // cnst138      watt   = HPE × 746

  generateConstantArray("YearToS",       "+3.155695200000000000000000000000000000000000000000000000e+07"); // cnst139      second = year  × (365.2425 × 24 × 3600)

  generateConstantArray("LbfftToNm",     "+1.355817948331400400000000000000000000000000000000000000e+00"); // cnst140      Nm = lbf×ft × 9.80665 × 0.45359237 × 12 × 0.0254

  generateConstantArray("_108",          "-1.080000000000000000000000000000000000000000000000000000e+02"); // cnst141
  generateConstantArray("_4",            "-4.000000000000000000000000000000000000000000000000000000e+00"); // cnst142
  generateConstantArray("_1",            "-1.000000000000000000000000000000000000000000000000000000e+00"); // cnst143
  generateConstantArray("_1oneE",        "-3.678794411714423215955237701614608674458111310317678345e-01"); // cnst144
  generateConstantArray("_7on20",        "-3.500000000000000000000000000000000000000000000000000000e-01"); // cnst145
  generateConstantArray("_1on4",         "-2.500000000000000000000000000000000000000000000000000000e-01"); // cnst146
  generateConstantArray("1e_49",         "+1.000000000000000000000000000000000000000000000000000000e-49"); // cnst147
  generateConstantArray("1e_37",         "+1.000000000000000000000000000000000000000000000000000000e-37"); // cnst148
  generateConstantArray("1e_24",         "+1.000000000000000000000000000000000000000000000000000000e-24"); // cnst149
  generateConstantArray("1e_6",          "+1.000000000000000000000000000000000000000000000000000000e-06"); // cnst150
  generateConstantArray("1e_4",          "+1.000000000000000000000000000000000000000000000000000000e-04"); // cnst151
  generateConstantArray("9on160",        "+5.625000000000000000000000000000000000000000000000000000e-02"); // cnst152
  generateConstantArray("1on10",         "+1.000000000000000000000000000000000000000000000000000000e-01"); // cnst153
  generateConstantArray("1on8",          "+1.250000000000000000000000000000000000000000000000000000e-01"); // cnst154
  generateConstantArray("5on24",         "+2.083333333333333333333333333333333333333333333333333333e-01"); // cnst155
  generateConstantArray("111on500",      "+2.220000000000000000000000000000000000000000000000000000e-01"); // cnst156
  generateConstantArray("1on4",          "+2.500000000000000000000000000000000000000000000000000000e-01"); // cnst157
  generateConstantArray("33on125",       "+2.640000000000000000000000000000000000000000000000000000e-01"); // cnst158
  generateConstantArray("1on3",          "+3.333333333333333333333333333333333333333333333333333333e-01"); // cnst159
  generateConstantArray("1on2",          "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst160
  generateConstantArray("2on3",          "+6.666666666666666666666666666666666666666666666666666667e-01"); // cnst161
  generateConstantArray("egamma",        "+5.772156649015328606065120900824024310421593359399235988e-01"); // cnst162
  generateConstantArray("ln2",           "+6.931471805599453094172321214581765680755001343602552541e-01"); // cnst163
  generateConstantArray("root2on2",      "+7.071067811865475244008443621048490392848359376884740366e-01"); // cnst164
  generateConstantArray("piOn4",         "+7.853981633974483096156608458198757210492923498437764552e-01"); // cnst165
  generateConstantArray("root3on2",      "+8.660254037844386467637231707529361834714026269051903140e-01"); // cnst166
  generateConstantArray("9on10",         "+9.000000000000000000000000000000000000000000000000000000e-01"); // cnst167
  generateConstantArray("ln2piOn2",      "+9.189385332046727417803297364056176398613974736377834128e-01"); // cnst168
  generateConstantArray("1",             "+1.000000000000000000000000000000000000000000000000000000e+00"); // cnst169
  generateConstantArray("3on2",          "+1.500000000000000000000000000000000000000000000000000000e+00"); // cnst170
  generateConstantArray("piOn2",         "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst171
  generateConstantArray("9on5",          "+1.800000000000000000000000000000000000000000000000000000e+00"); // cnst172
  generateConstantArray("19on10",        "+1.900000000000000000000000000000000000000000000000000000e+00"); // cnst173
  generateConstantArray("2",             "+2.000000000000000000000000000000000000000000000000000000e+00"); // cnst174
  generateConstantArray("ln10",          "+2.302585092994045684017991454684364207601101488628772976e+00"); // cnst175
  generateConstantArray("3piOn4",        "+2.356194490192344928846982537459627163147877049531329366e+00"); // cnst176
  generateConstantArray("3",             "+3.000000000000000000000000000000000000000000000000000000e+00"); // cnst177
  generateConstantArray("pi",            "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst178

  generateConstantArray("4",             "+4.000000000000000000000000000000000000000000000000000000e+00"); // cnst179
  generateConstantArray("3piOn2",        "+4.712388980384689857693965074919254326295754099062658731e+00"); // cnst180
  generateConstantArray("5",             "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst181
  generateConstantArray("6",             "+6.000000000000000000000000000000000000000000000000000000e+00"); // cnst182
  generateConstantArray("2pi",           "+6.283185307179586476925286766559005768394338798750211642e+00"); // cnst183
  generateConstantArray("7",             "+7.000000000000000000000000000000000000000000000000000000e+00"); // cnst184
  ...
  //generateConstantArray("21",            "+2.100000000000000000000000000000000000000000000000000000e+01"); // cnst191     used for Lanczos N=22
  //generateConstantArray("gammaR",        "+2.311891000000000000000000000000000000000000000000000000e+01"); // cnst192     used for Lanczos N=22
  generateConstantArray("24",            "+2.400000000000000000000000000000000000000000000000000000e+01"); // cnst191
  generateConstantArray("25",            "+2.500000000000000000000000000000000000000000000000000000e+01"); // cnst192
  generateConstantArray("27",            "+2.700000000000000000000000000000000000000000000000000000e+01"); // cnst193
  generateConstantArray("29",            "+2.900000000000000000000000000000000000000000000000000000e+01"); // cnst194     used for Lanczos N=30
  generateConstantArray("30",            "+3.000000000000000000000000000000000000000000000000000000e+01"); // cnst195
  generateConstantArray("gammaR",        "+3.143188335932791233062744140625000000000000000000000000e+01"); // cnst196     used for Lanczos N=30
  generateConstantArray("32",            "+3.200000000000000000000000000000000000000000000000000000e+01"); // cnst197
  generateConstantArray("36",            "+3.600000000000000000000000000000000000000000000000000000e+01"); // cnst198
  generateConstantArray("47",            "+4.700000000000000000000000000000000000000000000000000000e+01"); // cnst199
  generateConstantArray("180onPi",       "+5.729577951308232087679815481410517033240547246656432155e+01"); // cnst200
  generateConstantArray("60",            "+6.000000000000000000000000000000000000000000000000000000e+01"); // cnst201
  generateConstantArray("200onPi",       "+6.366197723675813430755350534900574481378385829618257950e+01"); // cnst202
  generateConstantArray("125",           "+1.250000000000000000000000000000000000000000000000000000e+02"); // cnst203
  generateConstantArray("150",           "+1.500000000000000000000000000000000000000000000000000000e+02"); // cnst204
  generateConstantArray("205",           "+2.050000000000000000000000000000000000000000000000000000e+02"); // cnst205
  generateConstantArray("360",           "+3.600000000000000000000000000000000000000000000000000000e+02"); // cnst206
  generateConstantArray("400",           "+4.000000000000000000000000000000000000000000000000000000e+02"); // cnst207
  generateConstantArray("600",           "+6.000000000000000000000000000000000000000000000000000000e+02"); // cnst208
  generateConstantArray("1000",          "+1.000000000000000000000000000000000000000000000000000000e+03"); // cnst209
  generateConstantArray("3200onPi",      "+1.018591635788130148920856085584091917020541732738921272e+03"); // cnst210
  generateConstantArray("1260",          "+1.260000000000000000000000000000000000000000000000000000e+03"); // cnst211
  generateConstantArray("1680",          "+1.680000000000000000000000000000000000000000000000000000e+03"); // cnst212
  generateConstantArray("2100",          "+2.100000000000000000000000000000000000000000000000000000e+03"); // cnst213
  generateConstantArray("2520",          "+2.520000000000000000000000000000000000000000000000000000e+03"); // cnst214
  generateConstantArray("3600",          "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst215
  generateConstantArray("6000",          "+6.000000000000000000000000000000000000000000000000000000e+03"); // cnst216
  generateConstantArray("6400",          "+6.400000000000000000000000000000000000000000000000000000e+03"); // cnst217
  generateConstantArray("9000",          "+9.000000000000000000000000000000000000000000000000000000e+03"); // cnst218
  generateConstantArray("9999",          "+9.999000000000000000000000000000000000000000000000000000e+03"); // cnst219
  generateConstantArray("10000",         "+1.000000000000000000000000000000000000000000000000000000e+04"); // cnst220
  generateConstantArray("25200",         "+2.520000000000000000000000000000000000000000000000000000e+04"); // cnst221
  generateConstantArray("42000",         "+4.200000000000000000000000000000000000000000000000000000e+04"); // cnst222
  generateConstantArray("73766",         "+7.376600000000000000000000000000000000000000000000000000e+04"); // cnst223
  generateConstantArray("86400",         "+8.640000000000000000000000000000000000000000000000000000e+04"); // cnst224
  generateConstantArray("1e6",           "+1.000000000000000000000000000000000000000000000000000000e+06"); // cnst225
  generateConstantArray("1e8",           "+1.000000000000000000000000000000000000000000000000000000e+08"); // cnst226
  generateConstantArray("1e9",           "+1.000000000000000000000000000000000000000000000000000000e+09"); // cnst227
  generateConstantArray("2p32",          "+4.294967296000000000000000000000000000000000000000000000e+09"); // cnst228
  generateConstantArray("1e32",          "+1.000000000000000000000000000000000000000000000000000000e+32"); // cnst229
  generateConstantArray("1e_6143",       "+1.000000000000000000000000000000000000000000000000000000e-6143"); // cnst230
  generateConstantArray("1e_10000",      "+1.000000000000000000000000000000000000000000000000000000e-10000"); // cnst231
  generateConstantArray("995on1000",     "+9.950000000000000000000000000000000000000000000000000000e-01"); // cnst232
  generateConstantArray("1e_32",         "+1.000000000000000000000000000000000000000000000000000000e-32"); // cnst233  if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
  ...
  // source: https://www.vttoth.com/FILES/lanczos.tgz
  generateConstantArray51("gammaC00",      "+2.506628274631000502415765284811045253006986740036318756703152698088652909808141644264478158114643"); // cnst234
  generateConstantArray51("gammaC01",      "+90795313438554.12585760728412902207185494121818510422990562390199419594778421509725059250660477478"); // cnst235
  generateConstantArray51("gammaC02",      "-966788810976616.3165238613163922965882171247384377771690792930892852741666385482155753752741846215"); // cnst236
  generateConstantArray51("gammaC03",      "+4800584619652903.130520636355077553806266769741879659498830407152738371167934130745600044623388068"); // cnst237
  generateConstantArray51("gammaC04",      "-14765868961297540.27064904092821149556100711736690133148186310232345861145647352272488652687903457"); // cnst238
  generateConstantArray51("gammaC05",      "+31520009430682538.17457937985920649010992386050798974102315423368438398460245896094052856122203775"); // cnst239
  generateConstantArray51("gammaC06",      "-49582365010416661.09326008795178786489253227385559918969410907133030684376335533671631031226780693"); // cnst240
  generateConstantArray51("gammaC07",      "+59568927402677002.04904143939675965126139196724493177865501398609411221897535112902272116361691451"); // cnst241
  generateConstantArray51("gammaC08",      "-55906342816448463.66899438215049258933959227649607458054232171494476786360919120606779433224203661"); // cnst242
  generateConstantArray51("gammaC09",      "+41579705761882718.81836872026846865595091094440150772658809279820390684724850793073271091274939857"); // cnst243
  generateConstantArray51("gammaC10",      "-24720590950576683.15802797249415999638807846332716128189191944706094647468094435934791589404852933"); // cnst244
  generateConstantArray51("gammaC11",      "+11801286985635343.58575847383935274746074763562173560062003882605616928057523864988435228756829000"); // cnst245
  generateConstantArray51("gammaC12",      "-4528196760270806.995369621666200554735964133084677928395568191894066774034949226537269118163329489"); // cnst246
  generateConstantArray51("gammaC13",      "+1393637064384668.917157722908202033726675908093527246704252104958408303381715387740313008845141904"); // cnst247
  generateConstantArray51("gammaC14",      "-342325016766888.5479936955467723387327600568400337988293619211103486673308239810777313598627315000"); // cnst248
  generateConstantArray51("gammaC15",      "+66578560073752.67246403368251720854813222816576859988564202476567602534436134110254400104404942690"); // cnst249
  generateConstantArray51("gammaC16",      "-10138544793022.41604607361389789580035078467043905508354626409172360042771928364449573017241522124"); // cnst250
  generateConstantArray51("gammaC17",      "+1190960562789.022438954352946350698116714288406439572882388541923119338089933765849690628214181895"); // cnst251
  generateConstantArray51("gammaC18",      "-105857166683.4123935373722836022279857082446927458625016646619942749346232137590435016079220044575"); // cnst252
  generateConstantArray51("gammaC19",      "+6945291327.796451124796820522079456356395826950647769057580507091796596412483352351183618549540841"); // cnst253
  generateConstantArray51("gammaC20",      "-325841232.8464260929688459624630603325071713695605050282752915466327762939975965122976740632863654"); // cnst254
  generateConstantArray51("gammaC21",      "+10491089.04183867431328778246866673594812558539972730107563825597306012268992151854201423652554125"); // cnst255
  generateConstantArray51("gammaC22",      "-219652.9521075940388150629624539015343302845601375714110391179944415326942002537588845815284375311"); // cnst256
  generateConstantArray51("gammaC23",      "+2782.640500292405732331071582785081107276742114500310058012342169019346915906467717358069437084413"); // cnst257
  generateConstantArray51("gammaC24",      "-19.31769482393126228999283928711817461826517031491190754697778125203682359806685621504868658022287"); // cnst258
  generateConstantArray51("gammaC25",      "+0.063813226038364736490677427053094184825560153837400637303741419820260651820063721448175876091640"); // cnst259
  generateConstantArray51("gammaC26",      "-0.000081213345051388882076245151519040492731624505633194015094047296992491188162952243561414155660"); // cnst260
  generateConstantArray51("gammaC27",      "+0.000000028425884712306176285600873249949436913248124688912080334562388569408577988201187292514310"); // cnst261
  generateConstantArray51("gammaC28",      "-0.000000000001515686855682081535390179935440192138970398946754350022046202956603870401666503058594"); // cnst262
  generateConstantArray51("gammaC29",      "+0.000000000000000003727184110478268994444830355775133199862144652804253659035860755294511160286895"); // cnst263

  // Array angle180
  generateConstantArray51("pi_51",         "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst264
  generateConstantArray51("200",           "+2.000000000000000000000000000000000000000000000000000000e+02"); // cnst265
  generateConstantArray51("180",           "+1.800000000000000000000000000000000000000000000000000000e+02"); // cnst266
  generateConstantArray51("3200",          "+3.200000000000000000000000000000000000000000000000000000e+03"); // cnst267

  // Array angle90
  generateConstantArray51("piOn2_51",      "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst268
  generateConstantArray51("100",           "+1.000000000000000000000000000000000000000000000000000000e+02"); // cnst269
  generateConstantArray51("90",            "+9.000000000000000000000000000000000000000000000000000000e+01"); // cnst270
  generateConstantArray51("1600",          "+1.600000000000000000000000000000000000000000000000000000e+03"); // cnst271

  // Array angle45
  generateConstantArray51("piOn4_51",      "+7.853981633974483096156608458198757210492923498437764552e-01"); // cnst272
  generateConstantArray51("50",            "+5.000000000000000000000000000000000000000000000000000000e+01"); // cnst273
  generateConstantArray51("45",            "+4.500000000000000000000000000000000000000000000000000000e+01"); // cnst274
  generateConstantArray51("800",           "+8.000000000000000000000000000000000000000000000000000000e+02"); // cnst275
  ...
  generateConstantArray34("_4712",       "-4.712000000000000000000000000000000000000000000000000000e+03"); // cnst277
  generateConstantArray34("_1",          "-1.000000000000000000000000000000000000000000000000000000e+00"); // cnst278
  generateConstantArray34("0",           "+0.000000000000000000000000000000000000000000000000000000e+00"); // cnst279
  generateConstantArray34("1e_32",       "+1.000000000000000000000000000000000000000000000000000000e-32"); // cnst280
  generateConstantArray34("1e_6",        "+1.000000000000000000000000000000000000000000000000000000e-06"); // cnst281
  generateConstantArray34("1e_4",        "+1.000000000000000000000000000000000000000000000000000000e-04"); // cnst282
  generateConstantArray34("1on100",      "+1.000000000000000000000000000000000000000000000000000000e-02"); // cnst283
  generateConstantArray34("1on10",       "+1.000000000000000000000000000000000000000000000000000000e-01"); // cnst284
  generateConstantArray34("1on3",        "+3.333333333333333333333333333333333333333333333333333333e-01"); // cnst285
  generateConstantArray34("1on2",        "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst286
  generateConstantArray34("2on3",        "+6.666666666666666666666666666666666666666666666666666667e-01"); // cnst287
  generateConstantArray34("ln2",         "+6.931471805599453094172321214581765680755001343602552541e-01"); // cnst288
  generateConstantArray34("1",           "+1.000000000000000000000000000000000000000000000000000000e+00"); // cnst289
  generateConstantArray34("piOn2",       "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst290
  generateConstantArray34("2",           "+2.000000000000000000000000000000000000000000000000000000e+00"); // cnst291
  generateConstantArray34("ln10",        "+2.302585092994045684017991454684364207601101488628772976e+00"); // cnst292
  generateConstantArray34("3",           "+3.000000000000000000000000000000000000000000000000000000e+00"); // cnst293
  generateConstantArray34("pi",          "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst294
  generateConstantArray34("4",           "+4.000000000000000000000000000000000000000000000000000000e+00"); // cnst295
  generateConstantArray34("5",           "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst296
  generateConstantArray34("2pi",         "+6.283185307179586476925286766559005768394338798750211642e+00"); // cnst297
  generateConstantArray34("7",           "+7.000000000000000000000000000000000000000000000000000000e+00"); // cnst298
  generateConstantArray34("9",           "+9.000000000000000000000000000000000000000000000000000000e+00"); // cnst299
  generateConstantArray34("10",          "+1.000000000000000000000000000000000000000000000000000000e+01"); // cnst300
  generateConstantArray34("12",          "+1.200000000000000000000000000000000000000000000000000000e+01"); // cnst301
  generateConstantArray34("14",          "+1.400000000000000000000000000000000000000000000000000000e+01"); // cnst302
  generateConstantArray34("24",          "+2.400000000000000000000000000000000000000000000000000000e+01"); // cnst303
  generateConstantArray34("28",          "+2.800000000000000000000000000000000000000000000000000000e+01"); // cnst304
  generateConstantArray34("31",          "+3.100000000000000000000000000000000000000000000000000000e+01"); // cnst305
  generateConstantArray34("38",          "+3.800000000000000000000000000000000000000000000000000000e+01"); // cnst306
  generateConstantArray34("47",          "+4.700000000000000000000000000000000000000000000000000000e+01"); // cnst307
  generateConstantArray34("60",          "+6.000000000000000000000000000000000000000000000000000000e+01"); // cnst308
  generateConstantArray34("100",         "+1.000000000000000000000000000000000000000000000000000000e+02"); // cnst309
  generateConstantArray34("153",         "+1.530000000000000000000000000000000000000000000000000000e+02"); // cnst310
  generateConstantArray34("275",         "+2.750000000000000000000000000000000000000000000000000000e+02"); // cnst311
  generateConstantArray34("367",         "+3.670000000000000000000000000000000000000000000000000000e+02"); // cnst312
  generateConstantArray34("400",         "+4.000000000000000000000000000000000000000000000000000000e+02"); // cnst313
  generateConstantArray34("1000",        "+1.000000000000000000000000000000000000000000000000000000e+03"); // cnst314
  generateConstantArray34("1401",        "+1.401000000000000000000000000000000000000000000000000000e+03"); // cnst315
  generateConstantArray34("1461",        "+1.461000000000000000000000000000000000000000000000000000e+03"); // cnst316
  generateConstantArray34("3600",        "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst317
  generateConstantArray34("4716",        "+4.716000000000000000000000000000000000000000000000000000e+03"); // cnst318
  generateConstantArray34("4800",        "+4.800000000000000000000000000000000000000000000000000000e+03"); // cnst319
  generateConstantArray34("4900",        "+4.900000000000000000000000000000000000000000000000000000e+03"); // cnst320
  generateConstantArray34("5001",        "+5.001000000000000000000000000000000000000000000000000000e+03"); // cnst321
  generateConstantArray34("10000",       "+1.000000000000000000000000000000000000000000000000000000e+04"); // cnst322
  generateConstantArray34("20000",       "+2.000000000000000000000000000000000000000000000000000000e+04"); // cnst323
  generateConstantArray34("32075",       "+3.207500000000000000000000000000000000000000000000000000e+04"); // cnst324
  generateConstantArray34("43200",       "+4.320000000000000000000000000000000000000000000000000000e+04"); // cnst325
  generateConstantArray34("65535",       "+6.553500000000000000000000000000000000000000000000000000e+04"); // cnst326
  generateConstantArray34("86400",       "+8.640000000000000000000000000000000000000000000000000000e+04"); // cnst327
  generateConstantArray34("146097",      "+1.460970000000000000000000000000000000000000000000000000e+05"); // cnst328
  generateConstantArray34("274277",      "+2.742770000000000000000000000000000000000000000000000000e+05"); // cnst329
  generateConstantArray34("1e6",         "+1.000000000000000000000000000000000000000000000000000000e+06"); // cnst330
  generateConstantArray34("1729777",     "+1.729777000000000000000000000000000000000000000000000000e+06"); // cnst331
  generateConstantArray34("2p32",        "+4.294967296000000000000000000000000000000000000000000000e+09"); // cnst332
  generateConstantArray34("maxDate",     "+3.155695348699627200000000000000000000000000000000000000e+18"); // cnst333
  generateConstantArray34("maxTime",     "+3.600000000000000000000000000000000000000000000000000000e+19"); // cnst334  if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
