...
static void calculateEigenvectors(const any34Matrix_t *matrix, bool isComplex, real_t *a, real_t *q, real_t *r, real_t *eig, realContext_t *realContext) {
  ...
  if(matrix->header.matrixRows == matrix->header.matrixColumns) {
    ...
    if((unknownsToFill = allocWp43(size * 2 * REAL_SIZE_IN_BYTES * 2))) {
      for(k = 0; k < size; k++) {
        ...
        if((v = allocWp43(size * 2 * REAL_SIZE_IN_BYTES * 2))) {
          do {
          ...
          freeWp43(v, size * 2 * REAL_SIZE_IN_BYTES * 2);
          v = NULL;
        }
        else {
          displayCalcErrorMessage(ERROR_RAM_FULL, ERR_REGISTER_LINE, NIM_REGISTER_LINE);
...
void realEigenvectors(const real34Matrix_t *matrix, real34Matrix_t *res, real34Matrix_t *ires) {
  ...
  if(matrix->header.matrixRows == matrix->header.matrixColumns) {
    if((bulk = allocWp43(size * size * 4 * REAL_SIZE_IN_BYTES * 2 * 4))) {
      a   = bulk;
      ...
      freeWp43(bulk, size * size * 4 * REAL_SIZE_IN_BYTES * 2 * 4);
    }
    else {
      displayCalcErrorMessage(ERROR_RAM_FULL, ERR_REGISTER_LINE, NIM_REGISTER_LINE);
    }
...
#if !defined(TESTSUITE_BUILD)
  void callByVectorElement(bool (*real_f)(real34Matrix_t *), bool (*complex_f)(complex34Matrix_t *)) {
